...
public interface CobinhoodTradingAPI {
    ...
    Call<CobinResponse<Order>> getOrder(@Header("Authorization") String authorization, @Path("order_id") String orderId);


    // Get all trades originating from the specific order:

    @GET("/v1/trading/orders/{order_id}/trades")
    Call<CobinResponse<List<Trade>>> getTrades(@Header("Authorization") String authorization, @Path("order_id") String orderId);
    ...
    Call<CobinResponse> modifyOrder(@Header("Authorization") String authorization, @Header("nonce") long nonce, @Body Order order, @Path("order_id") String orderId);


    // Cancel a single order:

    @DELETE("/v1/trading/orders/{order_id}")
    Call<CobinResponse> cancelOrder(@Header("Authorization") String authorization, @Header("nonce") long nonce, @Path("order_id") String orderId);
    ...
    Call<CobinResponse<Trade>> getTrade(@Header("Authorization") String authorization, @Path("trade_id") String tradeId);
